...
NexT.motion.integrator = {
  ...
  runItemFallback(item) {
    if (typeof item.complete === 'function') item.complete();
    if (!item.targets) return;

    const targets = typeof item.targets === 'string'
      ? document.querySelectorAll(item.targets)
      : item.targets instanceof Element
        ? [item.targets]
        : [...item.targets];

    targets.forEach(target => {
      if (!target?.style) return;
      if (item.opacity !== undefined) target.style.opacity = item.opacity;
      if (item.top !== undefined) target.style.top = `${item.top}px`;
      if (item.scaleX) target.style.transform = `scaleX(${item.scaleX[1] ?? 1})`;
    });
  },
  fallback() {
    const sequences = CONFIG.motion.async ? this.queue : [this.queue.flat()];
    sequences.forEach(sequence => sequence.forEach(item => this.runItemFallback(item)));
  },
  bootstrap() {
